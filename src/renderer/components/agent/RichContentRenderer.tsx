...
function ImageContent({ item }: { item: ToolRichContent }) {
  ...
  if (!imageSrc) {
    return (
      <div className="p-3 bg-black/20 rounded-md border border-border text-text-muted text-xs">
...
function CodeContent({ item, maxHeight }: { item: ToolRichContent; maxHeight: string }) {
  ...
  return (
    <div className="bg-black/30 rounded-md border border-border overflow-hidden">
      <div className="flex items-center justify-between px-3 py-1.5 bg-white/5 border-b border-border">
        <span className="text-text-muted flex items-center gap-2 text-xs">
...
function FileContent({ item }: { item: ToolRichContent }) {
  ...
  return (
    <div
      className="flex items-center gap-3 p-3 bg-black/20 rounded-md border border-border hover:bg-black/30 cursor-pointer transition-colors"
      onClick={handleOpen}
    >
...
function LinkContent({ item }: { item: ToolRichContent }) {
  ...
  return (
    <div
      className="flex items-center gap-3 p-3 bg-black/20 rounded-md border border-border hover:bg-black/30 cursor-pointer transition-colors group"
      onClick={handleOpen}
    >
          ...
          <thead className="bg-white/5 sticky top-0">
            <tr>
              {headers.map((header, i) => (
                <th key={i} className="px-3 py-2 text-left font-medium text-text-secondary border-b border-border">
                  {header}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {rows.map((row, i) => (
              <tr key={i} className="hover:bg-white/5 transition-colors">
                {row.map((cell, j) => (
                  <td key={j} className="px-3 py-2 text-text-muted border-b border-border">
                    {cell}
                  </td>
                ))}
              </tr>
