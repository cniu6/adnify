...
import { getTools, buildSystemPrompt, getToolApprovalType } from '../agent/tools'
...
import { processToolCall } from './agent/toolExecutor'
    ...
    async (userMessage: string | any[]) => {
      ...
      if (workspacePath) {
        const checkpoint = await checkpointService.createCheckpoint(
          'user_message',
          `Before: "${String(userMessage).slice(0, 50)}..."`,
          []
        )
      ...
      while (shouldContinue && loopCount < getMaxToolLoops() && !abortRef.current) {
          ...
          onStream: (chunk) => {
            ...
            if (chunk.type === 'text' && chunk.content) {
              ...
              if (id && name) {
                ...
                // 将工具调用关联到当前消息
                state.linkToolCallToLastMessage(id)
              }
        ...
        if (finalContent) {
          ...
          if (lastMsg && lastMsg.role === 'assistant' && !lastMsg.toolCallId) {
            ...
            if (!lastMsgText.includes(finalContent) && lastMsgText.length === 0) {
              const state = useStore.getState()
        ...
        if (result.data?.toolCalls && result.data.toolCalls.length > 0 && chatMode === 'agent') {
          for (const toolCall of result.data.toolCalls) {
            ...
            shouldContinue = await processToolCall(toolCall, conversationMessages, {
              autoApprove,
              workspacePath,
              waitForApproval,
              updateToolCall,
